...
\title{Priority Multi Server Queue Under Stochastic Customer Class Switching}
...
\(M/M/c\) queue is presented. Simulation and Markov chain models are given that
describe the behaviour of such systems, and their stationarity is explored.
...
% TODO I would flip this around: see later suggestions about the ordering of
% things.
\end{abstract}
...
degeneration. 
%TODO Add a citation here to justify this?
% I thought it would be easy to do so but I found two papers:
% 1.  https://pubmed.ncbi.nlm.nih.gov/29052082/: "here were no relationships between the duration of participants' wait for hernia repair and the change in patients' self-reported health for hernia-specific outcomes or overall health-related quality of life."
% 2.  https://pubmed.ncbi.nlm.nih.gov/10411289/: "Neither general quality of life nor condition-specific health appeared to worsen with the duration of wait [...]"
Another example would be a prioritisation scheme that attempts a
trade-off between medical need and waiting times.
% TODO A reference here would help.
...
exist for such a queueing system?
To answer this question,
two Markov chain models are defined, which are used to find
...
Section~\ref{sec:simulation} discusses simulation logic required and the
...
overtook more people, and were `bumped up' the queue. During the observation
...
\begin{itemize}
  ...
  \item \cite{bilodeau22} (2022): Analytical (truncated) expressions for
      a two class delayed accumulating priority M/G/1 queue. Customer
        ...
        % TODO I'd put all the `M/<>/<>` in math mode.
\end{itemize}
...
\section{Dynamic class priority \(M/M/c\) queue}\label{sec:system}
...
% TODO Do we need to describe what priority is? Or is this fairly obvious...
...
% TODO I think leading wth ciw here is a mistake. I'd go with a general
% description of the logic (which is useful for anyone implement this) followed
% by the ciw discussion.
...
can happen during the simulation run. Examples of \textbf{B}-events include the
next customer arriving, the current customer in service finishing their service,
or the server going off duty. Examples of \textbf{C}-events include a customer
beginning service after another customer has left the server.
In the case of customers randomly changing priority classes while waiting, one
additional \textbf{B}-event and two additional \textbf{C}-event are included:
...
% TODO I would put the "base" example of the healthcare example here. This
% can be shown as an example of using the simulation (perhaps with some basic
% assumption of theta?) that gives some nonsense or some bad thing that leads to
% the NEED for the MC models?
% Otherwise I don't think you have a reason for the models (in the narrative).

\section{Markov Chain Models}\label{sec:makovchains}
The situation described in Section~\ref{sec:system} can be modelled
using two different Markov chains.
...
class present. This is useful for analysing system-wide statistics such as
average queue size.
...
in state $\underline{\mathbf{s}} \in S$ (dropping the time step index notation
...
% TODO I feel like the term 'work conserving' comes out of nowhere here? Have I
% missed something?
...
one year of referrals, over 5 trials, take the amalgamated distribution of rank
differences, or overtakes, over the trials, and compare that with the
...
the indeterminate service discipline. Figure~\ref{fig:fitting_theta} compares
the modelled and actual distributions of net rank difference, along with the
Wasserstein metric $W$, for all pairs
...
customer class. This exploration may be useful to queue controllers, that is
