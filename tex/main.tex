...
All elements $\theta_{i,i+1}$ represent the direct downgrade rates; all elements
$\theta_{i,i-1}$ represent the direct upgrade rates, while all other elements
can be thought of as `skip-grades`.
This is shown in Figure~\ref{fig:skipgrades}.
% TODO: I think in the THETA matrix, the upgrades should be on the lower left, and the downgrades on the upper right.
...
\end{figure}
% TODO: Maybe use larger font here
...
% TODO: Maybe specify here that $\delta$ can also be expressed as:
% $\underline{\mathbf{\delta}} = (\delta_1, \delta_2, \dots, \delta_n)$
% Could be useful for equation \ref{eq:transitions}
...
% TODO: Should specify somewhere that c is the number of servers
...
% TODO: Shouldn't this be something like that? (I might be completely wrong)
% $S_b = {s \in \underline{\mathbf{s}} | s_i = b for all i \in [0, K-1]}$
...
% TODO: Idea - what if we create another Markov model based only on Theta? Each state is one of the possible classes and the rates between states are the theta_{i,j}'s. That way by getting the state probabilities we can see what's the actual probability of being at each class. Then we could argue that these probabilities could be used together with each class's service rate \mu_k to find something like the relative traffic intensity for this particular model. Not sure if this is a good idea.
