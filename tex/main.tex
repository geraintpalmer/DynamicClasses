...
\textcolor{red}{In queueing systems, `priority' usually denotes some measure of
a customers' urgency. Here we precisely define a customers' priority by an
...
There are a \textcolor{red}{several} situations in which a customer's priority
in a queue might change \textcolor{red}{while they wait}, or equivalently where
their priority depends on the amount of time they have already spent in the queue.
...
downgrade over time: consider \textcolor{red}{waiting for some} medical intervention that can improve a
patient's outcome \textcolor{red}{only} if caught early: if a patient has been waiting a long time
...
their class to any other class at specific rates. Thus upgrades and downgrades
are modelled.
...
finite population queue with randomly increasing priority numbers is described
...
Appendix~\ref{apx:simulation_logic} gives an overview of the event scheduling approach and details the adaptations required for priority switching logic.}
...
referrals per day, or one referral every 2.16 days.} Service rate data is
estimated to be around 0.5 a day, \textcolor{red}{that is around one endoscopy
procedure every other day}.
To determine the appropriateness of the stochastic
...
downgraded and then upgraded during their waiting time.
...
$\theta_{21}$. For each parameter set we observe one year of referrals, over 5
trials. One KPI of interest is the amalgamated distribution of rank differences,
or overtakes, over the trials; and we compare that with the
...
finishing services, and case~\ref{eqn:transitions_switching} denotes transitions
representing customers switching priorities.}
...
\textcolor{red}{Here cases correspond to arrivals, services, and priority switching, all in relation to the customer currently under consideration. This customer's class in $n$, which is subject to change.
Cases~\ref{eqn:transitions_sojourn_myservice}, and~\ref{eqn:transitions_sojourn_switch_me} correspond to transitions that affect the customer under consideration, finishing service and switching priority respectively.
Cases~\ref{eqn:transitions_sojourn_arrival_myclass} and~\ref{eqn:transitions_sojourn_arrival_otherclass} correspond to arrivals of other customers, of the same and different class to the considered individual, respectively.
Cases~\ref{eqn:transitions_sojourn_service_myclass} and~\ref{eqn:transitions_sojourn_service_otherclass} correspond to services of other customers, of the same and different class to the considered individual, respectively.
Cases~\ref{eqn:transitions_sojourn_switch_othermy}, \ref{eqn:transitions_sojourn_switch_myother}, and~\ref{eqn:transitions_sojourn_switch_otherother} all correspond to customers switching priorities, to $n$, from $n$, and from two classes neither of which are $n$, respectively.}
...
evaluate, and for a complete CDF Equation~\ref{eqn:sojourn_time_cdf} would need
to be evaluated many times for each value of $x$. Additionally, these CDF
equations have no known easily computable inverse. However, matrix methods can
give us summary statistics. \cite{stewart09}.}
...
Equation~\ref{eqn:mean_sojourn} and the mean sojourn time for customers arriving
as class $k$ by Equation~\ref{eqn:mean_sojourn_class_k}.
...
16 times the variance without priority switching, demonstrating that some
important performance measures are not captured by FIFO alone.
