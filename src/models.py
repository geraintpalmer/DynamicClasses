...
class MarkovChain:
    ...
    """
    def __init__(self, number_of_servers, arrival_rates, service_rates, class_change_rate_matrix, inf, sojourn=False):
        """
        ...
        if sojourn:
            self.SojournTime_MC = MarkovChain_SoujournTime(number_of_servers, arrival_rates, service_rates, class_change_rate_matrix, inf)
...
class MarkovChain_SoujournTime:
    ...
    def __init__(self, number_of_servers, arrival_rates, service_rates, class_change_rate_matrix, inf):
        ...
        self.inf = inf
        self.k = len(arrival_rates)
        self.State_Space = list(itertools.product(*([range(self.inf) for _ in range(self.k)] + [range(self.inf)] + [range(self.k)]))) + ['*']
        self.lenmat = len(self.State_Space)
        self.write_transition_matrix()
        self.discretise_transition_matrix()
        self.solve()
    ...
    def find_transition_rates(self, state1, state2):
        ...
        n = state1[-1]
        if state2 == '*':
            if sum(state1[:n+1]) < self.c: # I'm currently in service, when will I end service?
                return self.ms[n]
            else:
                return 0
    ...
    def solve(self):
        ...
        self.mean_sojourn_time = {self.State_Space[i]: float(time2absorb[i]) for i in range(len(time2absorb))}
...
class Simulation:
    ...
    def find_mean_sojourn_time_by_class(self):
        ...
        self.mean_sojourn_times_by_class = {}
        for clss in range(self.k):
            self.mean_sojourn_times_by_class[clss] = np.mean([r.waiting_time + r.service_time for r in recs if r.original_customer_class == clss])
